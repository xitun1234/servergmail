...
class DataAccountModel {
    ...
    getScript(deviceName) {
        const url = `http://reglzd.herokuapp.com/api/getKichBan&devicename=${deviceName}`;

        this.axiosGet(url).then((returlVal) => {

            this.scriptName = returlVal.data.scriptName;
        }).catch(error => {
            return Promise.reject(error);
        });
    }
    ...
    resetZaloPay() {
        at.appKill("com.tigisoftware.ADManager");
        at.usleep(1000000);
        ...
        this.AtHelper.thongBao("Keyword: ZaloPay");
        this.AtHelper.inputTextCustom("ZaloPay");
        at.usleep(2000000);
    ...
    resetTiki() {
        ...
        this.AtHelper.thongBao("Keyword: Tiki");
        this.AtHelper.inputTextCustom("Tiki");
        at.usleep(2000000);
    ...
    resetAndRunZalo() {
        //run app appKill("com.apple.mobilesafari")
        at.appKill("com.tigisoftware.ADManager");
        at.usleep(1000000);
        this.AtHelper.thongBao('run App Manager');
        at.appRun("com.tigisoftware.ADManager");
    ...
    runZaloPayByOTP() {
        ...
        const findColorCall = this.findMau(130, 1139, 28159, 2);

        if (findColorCall.success == true) {
            at.toast("Click button Dang Nhap Voi Zalo");

            at.usleep(1000000);
            at.touchDown(0, findColorCall.data.x, findColorCall.data.y)
            at.usleep(50000)
            at.touchUp(0, findColorCall.data.x, findColorCall.data.y);

            const findIconX = this.findMau(630, 1279, 0, 2);
            if (findIconX.success == true) {

                this.AtHelper.thongBao("Chuan bi dien phoneNumber");
                at.usleep(1000000);

                at.touchDown(0, findIconX.data.x, findIconX.data.y)
                at.usleep(50000)
                at.touchUp(0, findIconX.data.x, findIconX.data.y);
                at.usleep(1000000);
                at.touchDown(0, findIconX.data.x, findIconX.data.y)
                at.usleep(50000)
                at.touchUp(0, findIconX.data.x, findIconX.data.y);
                ...
                const findColorBtnTiepTuc = this.findMau(383, 826, 27391, 2);
                if (findColorBtnTiepTuc.success = true) {
                    this.AtHelper.thongBao("Click btn Tiep Tuc");
                    at.usleep(1000000);

                    at.touchDown(0, findColorBtnTiepTuc.data.x, findColorBtnTiepTuc.data.y)
                    at.usleep(50000)
                    at.touchUp(0, findColorBtnTiepTuc.data.x, findColorBtnTiepTuc.data.y);
                    at.usleep(1000000);
                }
            }
        }
    ...
    dienDiaChiTikiTH2() {
        ...
        if (find_btnGiao.success == true) {
            ...
            //input diachi
            const diaChiTemp = Math.floor(Math.random() * 100).toString() + "/" + Math.floor(Math.random() * 11).toString() + " to hien thanh";
            this.AtHelper.thongBao("Input Dia chi: " + diaChiTemp);
            ...
            this.AtHelper.inputTextCustom(diaChiTemp);
            at.usleep(2000000);
    ...
    dienMatKhauZLP() {
        this.AtHelper.thongBao("Dien mat khau zlp 112233");
        const find_IconX = this.findMau(631, 1279, 0, 2);

        if (find_IconX.success == true) {
            this.AtHelper.thongBao("Dien mat khau zlp 112233");

            //11
            at.touchDown(0, 131, 955);
            at.usleep(50000);
            at.touchUp(0, 131, 955);
            at.usleep(1000000);
            ...
            //22
            at.touchDown(0, 373, 955);
            at.usleep(50000);
            at.touchUp(0, 373, 955);
            at.usleep(1000000);
            ...
            //33
            at.touchDown(0, 625, 953);
            at.usleep(50000);
            at.touchUp(0, 625, 953);
            at.usleep(1000000);
    ...
    tikiSearch(keyword) {
        ...
        this.AtHelper.thongBao("Thuc hien input tu khooa " + keyword);
        at.appRun("vn.tiki.app.Tiki");
        at.usleep(2000000);

        const find_ColorBack = this.findMau(45, 83, 16777215, 2);

        if (find_ColorBack.success == true) {
            at.touchDown(0, find_ColorBack.data.x, find_ColorBack.data.y);
            at.usleep(50000);
            at.touchUp(0, find_ColorBack.data.x, find_ColorBack.data.y);
            at.usleep(1000000);

            const find_KinhLup = this.findMau(79, 165, 10066329, 2);
            if (find_KinhLup.success == true) {
                this.AtHelper.thongBao("Click tim kiem");
                at.touchDown(0, find_KinhLup.data.x, find_KinhLup.data.y);
                at.usleep(50000);
                at.touchUp(0, find_KinhLup.data.x, find_KinhLup.data.y);
                at.usleep(1000000);

                this.AtHelper.inputTextCustom(keyword);
                at.usleep(2000000);
                at.tap(351, 1284);

                const find_BtnTim = this.findMau(631, 1285, 31487, 2);
                if (find_BtnTim.success == true) {
                    this.AtHelper.thongBao("Click btn Tim");
                    at.touchDown(0, find_BtnTim.data.x, find_BtnTim.data.y);
                    at.usleep(50000);
                    at.touchUp(0, find_BtnTim.data.x, find_BtnTim.data.y);
                    at.usleep(1000000);
                }
            }
        }
    ...
    tikiThemGioHang() {
        this.AtHelper.thongBao("Thuc hien them gio hang");
        at.appRun("vn.tiki.app.Tiki");
        at.usleep(2000000);

        const find_ChonMua = this.findMau(382, 1265, 16728654, 2);
        if (find_ChonMua.success == true) {
            this.AtHelper.thongBao("Click Chon mua");
            at.touchDown(0, find_ChonMua.data.x, find_ChonMua.data.y);
            at.usleep(50000);
            at.touchUp(0, find_ChonMua.data.x, find_ChonMua.data.y);
            at.usleep(2000000);

            const find_ThemGioHang = this.findMau(394, 1266, 16728654, 2);
            if (find_ThemGioHang.success == true) {
                this.AtHelper.thongBao("Click Them Gio Hang");
                at.touchDown(0, find_ThemGioHang.data.x, find_ThemGioHang.data.y);
                at.usleep(50000);
                at.touchUp(0, find_ThemGioHang.data.x, find_ThemGioHang.data.y);
                at.usleep(1000000);
            }
        }
    }

    tikiChonVoucher(voucher1, voucher2) {
        this.AtHelper.thongBao("Thuc hien nhap voucher");
        at.appRun("vn.tiki.app.Tiki");
        at.usleep(2000000);

        const find_ApDung = this.findMau(612, 986, 875702, 2);
        if (find_ApDung.success == true) {
            this.AtHelper.thongBao("Click input voucher");
            at.touchDown(0, 211, 989);
            at.usleep(50000);
            at.touchUp(0, 211, 989);
            at.usleep(1000000);

            this.AtHelper.inputTextCustom(voucher1);
            at.usleep(2000000);

            const find_ApDung1 = this.findMau(609, 610, 875702, 2);
            if (find_ApDung1.success == true) {
                this.AtHelper.thongBao("Click ap dung");
                at.touchDown(0, find_ApDung1.data.x, find_ApDung1.data.y);
                at.usleep(50000);
                at.touchUp(0, find_ApDung1.data.x, find_ApDung1.data.y);
                at.usleep(1000000);

                this.AtHelper.thongBao("Click input voucher Zalo Pay");
                at.touchDown(0, 251, 789);
                at.usleep(50000);
                at.touchUp(0, 251, 789);
                at.usleep(1000000);

                this.AtHelper.inputTextCustom(voucher2);
                at.usleep(2000000);
    ...
    lzdSearch(keyword) {
        ...
        const find_Back = this.findMau(37, 83, 4408131, 2);
        if (find_Back.success == true) {
            this.AtHelper.thongBao("Click go back");
            at.touchDown(0, find_Back.data.x, find_Back.data.y);
            at.usleep(50000);
            at.touchUp(0, find_Back.data.x, find_Back.data.y);
            at.usleep(1000000);

            const find_HopQua = this.findMau(376, 668, 16263534, 2);
            if (find_HopQua.success == true) {
                this.AtHelper.thongBao("Click tat hop qua");
                at.touchDown(0, 102, 250);
                at.usleep(50000);
                at.touchUp(0, 102, 250);
                at.usleep(1000000);
            }

            const find_KinhLup = this.findMau(136, 81, 16777215, 2);
            if (find_KinhLup.success == true) {
                this.AtHelper.thongBao("Click Tim kiem");
                at.touchDown(0, 222, 89);
                at.usleep(50000);
                at.touchUp(0, 222, 89);
                at.usleep(2000000);
                ...
                const find_Tim = this.findMau(657, 1283, 31487, 2);
                if (find_Tim.success == true) {
                    this.AtHelper.thongBao("Click Tim");
                    at.touchDown(0, find_Tim.data.x, find_Tim.data.y);
                    at.usleep(50000);
                    at.touchUp(0, find_Tim.data.x, find_Tim.data.y);
                    at.usleep(1000000);
                }
            }
            else {
                const find_KinhLupXam = this.findMau(136, 81, 10066329, 2);
                if (find_KinhLupXam.success == true) {
                    this.AtHelper.thongBao("Click Kinh Lup");
                    at.touchDown(0, find_KinhLupXam.data.x, find_KinhLupXam.data.y);
                    at.usleep(50000);
                    at.touchUp(0, find_KinhLupXam.data.x, find_KinhLupXam.data.y);
                    at.usleep(1000000);
    ...
    luuRRS() {
        this.AtHelper.thongBao("Open App XoaInfo");
        at.appRun("com.ienthach.XoaInfo");
        this.AtHelper.delayCus(3);

        this.AtHelper.thongBao("Click input RRSName");
        at.tap(572, 929);
        this.AtHelper.delayCus(1);
        at.tap(572, 1152);
        this.AtHelper.delayCus(2);
        ...
        touchDown(5, 691.78, 1170.28);
        usleep(4000000);
        touchUp(5, 691.78, 1170.28);

        this.AtHelper.delayCus(1);
        at.inputText("lzd " + this.username);
        this.AtHelper.delayCus(2);
        ...
        const optionLuuRRS = {
            colors: [ // REQUIRED, colors and their relative positions
                ...
                { color: 16777215, x: 1, y: 28 },
                { color: 16777215, x: 30, y: 30 },
                { color: 16777215, x: 24, y: 1 },
                { color: 0, x: 15, y: 17 },
                { color: 0, x: 15, y: 15 }
            ],
            count: 3, // OPTIONAL, default is 0, 0 means no limitation
            region: { x: 88.73, y: 181.69, width: 237.68, height: 141.55 }, // OPTIONAL, default is null, null means the whole screen
            debug: false,
        }
        const find_optionLuuRRS = this.AtHelper.findToaDo(optionLuuRRS, 2);
        if (find_optionLuuRRS.success == true) {
            this.AtHelper.thongBao("Click btn DangNhap");
            at.tap(find_optionLuuRRS.data.x, find_optionLuuRRS.data.y);
        }
        else {
            at.tap(205, 250);
        }
